...
var clean_text = function(text_value){
	return text_value.replace(/\n|\r/g, " ").replace("\t", " ").replace(/^\s+|\s+$/g, '').replace(/ +/g, ' ').replace(/[,%\$]/g, '').replace(/\?/g, 0);
}

var parse_url = function(date_, body){
	var jQuery = cheerio.load(body);
	var total_market = jQuery("#total-marketcap").text().split(":")[1];
	jQuery("tr").each(
		function(tr_index, tr){
			...
			jQuery("td", tr).each(
				function(td_index, td){
					row.push(
						clean_text(jQuery(td).text())
					);
				}
			)
			row.push(clean_text(total_market))
			var tmp_row = [date_,].concat(row);
			if(tmp_row[1] && tmp_row[1].match(/^[0-9]+$/) && tmp_row.length > 6){
				console.log(tmp_row.join("\t"));
			}
		}
	)
}
...
var main = function(){
	async.map(urls, function(url, callback){
			try{
				var date_ = url.split("/historical/")[1].split("/")[0];
			}catch(err){
				var date_ = new Date().toISOString().split("T")[0]
			}
			var date_formatted = date_.substring(0,4) + "-" + date_.substring(4,6)+ "-" + date_.substring(6,8);
			get_url(url, function(err, body){
				parse_url(date_formatted, body);
				callback(null, url);
			});
